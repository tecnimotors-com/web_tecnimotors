    ...
    public class MaestroArticuloRepository(PostgreSQLConfiguration connectionString) : IMaestroArticuloRepository
    {
        ...
        public async Task<IEnumerable<TlArticulo>> GetArticulosAsync()
        {
            ...
            var sql = @"
                        ...
                        CASE WHEN split_part(subquery.marca, ' ', 1) LIKE 'K%' THEN 'KENDA' ELSE '' END AS marcaoriginal,
                        CASE WHEN split_part(subquery.marca, ' ', 1) LIKE 'K%' THEN split_part(subquery.marca, ' ', 1) ELSE '' END AS cocada,
                        CASE WHEN split_part(subquery.marca, ' ', 2) LIKE '%/%' THEN split_part(split_part(subquery.marca, ' ', 2),'/',1) ELSE '' END AS ancho,
                        CASE WHEN split_part(subquery.marca, ' ', 2) LIKE '%-%' THEN (case when split_part(split_part(subquery.marca, ' ', 2), '-', 1) like '%/%' THEN split_part(split_part(split_part(subquery.marca, ' ', 2), '-', 1),'/',2) ELSE split_part(split_part(subquery.marca, ' ', 2), '-', 1) END  )
	                        ELSE (CASE WHEN split_part(subquery.marca, ' ', 2) LIKE '%/%' THEN (case when split_part(split_part(subquery.marca, ' ', 2), '-', 1) like '%/%' THEN split_part(split_part(split_part(subquery.marca, ' ', 2), '-', 1),'/',2) ELSE split_part(split_part(subquery.marca, ' ', 2), '-', 1) END  ) END) END AS perfil,
                        CASE WHEN split_part(subquery.marca, ' ', 2) LIKE '%-%' THEN split_part(split_part(subquery.marca, ' ', 2), '-', 2) 
	                        ELSE '' END AS aro,
                        CASE WHEN subquery.marca LIKE '%TT%' THEN 'TT' WHEN subquery.marca LIKE '%TL%' THEN 'TL' ELSE '' END AS tipouso
                        FROM (SELECT id, marca AS marca, codigo, descripcion FROM public.maestro_articulos where descripcion LIKE '%KENDA%' AND tipo between '02' and '03'  AND familia = '003' 
                        AND codigo_equivalente LIKE '%K%' AND TRIM(descripcion) = TRIM('LLANTA PISTA KENDA') AND marca like 'K%' order by id asc) AS subquery
                       ";
