...
@Component({
  selector: 'app-listadoaceite',
  imports: [SharedMain, PreloaderComponent],
  templateUrl: './listadoaceite.component.html',
  styleUrls: ['./listadoaceite.component.scss'],
})
export class ListadoaceiteComponent implements OnInit {
  public txtcategoria: string = localStorage.getItem('categoriaaceite') || '';
  ...
  public lstListarTipoAceite: any[] = [];
  ...
  constructor(
    private maestroclasi: MaestroclasificadoService,
  ...
  ListadoTipoAceite() {
    this.maestroclasi.getTipoMarcaAceite().subscribe({
      next: (lst: any) => {
        this.lstListarTipoAceite = lst;
      },
    });
  }
  ...
  ListarGeneralAceite() {
    const frombody: any = {
      tipoMarca: this.txtcategoria ?? '',
    };
    this.showAlert('Cargando Información....', 'info');
    this.maestroclasi.getListadoGeneralAceite(frombody).subscribe({
      next: (lst: any) => {
  ...
  MdAceitelistado(content: any, item: any) {
