...
export class ListadollantasComponent implements OnInit {
  ...
  public p: number = parseInt(localStorage.getItem('pllanta')!) || 1;
  ...
  loadFilters() {
    this.filters.medida = localStorage.getItem('medidallanta') || '';
    this.filters.modelo = localStorage.getItem('modelollanta') || '';
    this.filters.marca = localStorage.getItem('marcallanta') || '';
    this.filters.categoria = localStorage.getItem('categoriallanta') || '';
  }
  saveFilters() {
    localStorage.setItem('medidallanta', this.filters.medida);
    localStorage.setItem('modelollanta', this.filters.modelo);
    localStorage.setItem('marcallanta', this.filters.marca);
    localStorage.setItem('categoriallanta', this.filters.categoria);
  }
  onFilterChange() {
    this.showAlert('Cargando Información', 'info');
    this.servicesmaestro.getListadoLLanta(this.filters).subscribe({
      next: (data) => {
        this.ListMedida = data.medidas;
        this.ListModelo = data.modelos;
        this.ListMarca = data.marcas;
        this.ListCategoria = data.categorias;
        this.ListadoGeneral = data.generallanta;
        this.saveFilters();
        this.showAlert('Se cargo exitosamente la Información', 'success');
      },
      error: (err) => {
  ...
  Paginacion() {
    localStorage.setItem('pllanta', this.p.toString());
  }
  ...
  ClearCategoria() {
    ...
    localStorage.setItem('categoriallanta', '');
    this.onFilterChange();
  }
  ClearMarca() {
    this.filters.marca = '';
    localStorage.setItem('marcallanta', '');
    this.onFilterChange();
  }
  ClearModelo() {
    this.filters.modelo = '';
    localStorage.setItem('modelollanta', '');
    this.onFilterChange();
  }
  ClearMedida() {
    this.filters.medida = '';
    localStorage.setItem('medidallanta', '');
    this.onFilterChange();
  }
}
